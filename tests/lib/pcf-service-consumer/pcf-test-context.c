...
static pcf_test_context_t self;

int __pcf_test_log_domain;

static OGS_POOL(pcf_test_sess_pool, pcf_test_sess_t);

static int context_initialized = 0;

static void clear_pcf_app_session_id(pcf_test_sess_t *sess);

void pcf_test_context_init(void)
{
    ogs_assert(context_initialized == 0);

    /* Initialize AF context */
    memset(&self, 0, sizeof(pcf_test_context_t));

    ogs_log_install_domain(&__pcf_test_log_domain, "pcf-test", ogs_core()->log.level);
    af_context_init();
    
    ogs_pool_init(&pcf_test_sess_pool, ogs_app()->pool.sess);
...
void pcf_test_context_final(void)
{
    ogs_assert(context_initialized == 1);

    pcf_test_sess_remove_all();
    ...
    ogs_pool_final(&pcf_test_sess_pool);
...
pcf_test_context_t *pcf_test_self(void)
{
    return &self;
}

int pcf_test_context_parse_config(void)
{
    int rv;
...
pcf_test_sess_t *pcf_test_sess_add_by_ue_address(ogs_ip_t *ue_address)
{
    pcf_test_sess_t *sess = NULL;

    ogs_assert(ue_address);
    ogs_assert(ue_address->ipv4 || ue_address->ipv6);

    ogs_pool_alloc(&pcf_test_sess_pool, &sess);
    ogs_assert(sess);
    memset(sess, 0, sizeof *sess);

    sess->af_app_session_id = ogs_msprintf("%d",
            (int)ogs_pool_index(&pcf_test_sess_pool, sess));
...
void pcf_test_sess_remove(pcf_test_sess_t *sess)
{
    int i;
    ...
    ogs_pool_free(&pcf_test_sess_pool, sess);
}

void pcf_test_sess_remove_all(void)
{
    pcf_test_sess_t *sess = NULL, *next_sess = NULL;

    ogs_list_for_each_safe(&self.sess_list, next_sess, sess)
        pcf_test_sess_remove(sess);
}
...
bool pcf_test_sess_set_pcf_app_session_id(pcf_test_sess_t *sess, char *pcf_app_session_id)
{
    ogs_assert(sess);
...
pcf_test_sess_t *pcf_test_sess_find(uint32_t index)
{
    return ogs_pool_find(&pcf_test_sess_pool, index);
}

pcf_test_sess_t *pcf_test_sess_find_by_af_app_session_id(char *af_app_session_id)
{
    ogs_assert(af_app_session_id);
    return pcf_test_sess_find(atoll(af_app_session_id));
}

pcf_test_sess_t *pcf_test_sess_find_by_pcf_app_session_id(char *pcf_app_session_id)
{
    ogs_assert(pcf_app_session_id);
    return (pcf_test_sess_t *)ogs_hash_get(self.pcf_app_session_id_hash,
...
void pcf_test_sess_associate_pcf_client(pcf_test_sess_t *sess)
{
    ogs_sbi_client_t *client = NULL;
    ogs_sockaddr_t *addr = NULL;
